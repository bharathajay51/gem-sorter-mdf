...
void CollectCandy() {
  ...
  Serial.println("Collecting Candy");
  delay(collectDelay);
  TopServo.detach();
}
//********************************************
...
int SenseColour() {
  ...
  if (r == 65535 && g == 65535 && b == 65535) {
    ...
    tcs.begin();
    return -1; // Return -1 (Blank) on error
  }
  ...
  return colorAngle; // Return the color angle
}
//********************************************
  ...
  if (Serial.available()) {
    ...
    if (input.equalsIgnoreCase("s")) {
      ...
      do {
        ...
        Serial.print(count);
        Serial.println(" #############");
        CollectCandy(); // Rotate top disc to collect a candy
        delay(stepDelay);
        MoveCandyToSensor(); // Bring candy over the colour sensor
        delay(stepDelay);
        angle = SenseColour(); // Read and classify the colour
        delay(stepDelay);
        MoveSlideTo(angle); // Position the slide for this colour
        delay(stepDelay);
        DispenseCandy(); // Release the candy down the slide
        delay(stepDelay);
        Serial.println("#################################");
        Serial.println(" ");
        Serial.println(" ");
      ...
      Serial.print("Number of candy sorted : ");
